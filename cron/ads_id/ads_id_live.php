...
$conn = mysqli_connect("localhost", "ppzjbhawdp", "bmPY3k8g8d", "ppzjbhawdp");
...
$response['Total'] = 0;
$response['response'] = '';
$ads_again_details = "SELECT * FROM `ad_ids` WHERE update_status = 2";
...
if ($res->num_rows > 0) {
    ...
    while ($ad = $res->fetch_assoc()) {
        ...
        $filename = $_SERVER['DOCUMENT_ROOT'].'cron/Ads_id/'.$app_code.'_ad_id.json';
        $upd_ads_query = "UPDATE `ad_ids` SET `json_path`= '$app_code' where `id` = $get_id";
...
//
$ads_details = "SELECT * FROM `ad_ids` WHERE update_status = 1";
